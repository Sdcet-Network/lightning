...
from lightning.dispatcher.workers.base import DriverResult, WorkerDriver
...
def _resolve_pi_binary() -> str:
    ...
    for parent in Path(__file__).resolve().parents:
        candidate = parent / "bundled" / "pi" / "node_modules" / ".bin" / "pi"
...
class PiDriver(WorkerDriver):
    ...
    def _pi_binary(self) -> str:
        return _resolve_pi_binary() if self.local else "pi"
    ...
    def build_execute(self, worker: WorkerConfig, prompt: str, session: str | None) -> DriverResult:
        if self.local and not self._has_injected_provider(worker):
    ...
    @staticmethod
    def _has_injected_provider(worker: WorkerConfig) -> bool:
        env = worker.env
        return all(env.get(key) for key in ("PI_MODEL", "PI_BASE_URL", "PI_API_KEY", "PI_PROVIDER_API"))
    ...
    def _agent_dir(worker: WorkerConfig) -> str:
        return str(PurePosixPath("/tmp/lightning-pi") / worker.name)
    ...
    def _models_json(worker: WorkerConfig) -> str:
        ...
        payload = {"providers": {"lightning": provider}}
