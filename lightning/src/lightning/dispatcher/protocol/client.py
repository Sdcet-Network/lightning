...
from lightning.server.models import Intent, ModelProvider, ProjectDetail, ProjectSummary, Settings
...
class LightningClient:
    def __init__(self, base_url: str, timeout: float = 10.0):
        ...
        self._providers_adapter = TypeAdapter(list[ModelProvider])
    ...
    def get_providers(self) -> list[ModelProvider]:
        response = self._session().get(self._url("/settings/providers"), timeout=self._timeout)
        response.raise_for_status()
        return self._providers_adapter.validate_python(response.json())
