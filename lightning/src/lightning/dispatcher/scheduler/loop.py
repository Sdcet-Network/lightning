...
from lightning.dispatcher.models import ReasonCheckpoint, RunningTask
from lightning.dispatcher.protocol.client import LightningClient
from lightning.dispatcher.runtime.cancellation import TaskCancellation
from lightning.dispatcher.runtime.containers import ContainerManager
from lightning.dispatcher.runtime.local_backend import LocalBackend
from lightning.dispatcher.runtime.startup_healthcheck import format_failure_summary, run_startup_healthchecks
from lightning.dispatcher.scheduler.worker_select import choose_worker
from lightning.dispatcher.workers.registry import get_driver
from lightning.dispatcher.tasks.bootstrap import run_bootstrap_task
from lightning.dispatcher.tasks.explore import run_explore_task
from lightning.dispatcher.tasks.reason import run_reason_task
from lightning.server.models import Intent, ModelProvider, ProjectDetail, ProjectSummary
...
class DispatcherLoop:
    def __init__(self, config_path: Path):
        ...
        self.client = LightningClient(self.config.server)
    ...
    def _inject_providers(self) -> None:
        """Pull model providers from the server and inject them into pi workers.

        Matches the local-mode workflow where the server owns provider credentials:
        a pi worker without explicit ``PI_*`` env gets the pinned ``PI_PROVIDER``
        (or the first enabled provider) injected so it targets the configured LLM.
        """
        try:
            providers = self.client.get_providers()
        except requests.RequestException as exc:
            LOG.warning("failed to fetch model providers from server error=%s", exc)
            return
        ...
        for worker in self.config.workers:
            if worker.type != "pi":
                continue
            if any(worker.env.get(key) for key in ("PI_MODEL", "PI_BASE_URL", "PI_API_KEY", "PI_PROVIDER_API")):
                continue
            provider = self._select_provider(worker, enabled)
            if provider is None:
                continue
            worker.env["PI_MODEL"] = provider.model
            worker.env["PI_BASE_URL"] = provider.base_url.rstrip("/")
            worker.env["PI_API_KEY"] = provider.api_key
            worker.env["PI_PROVIDER_API"] = provider.api
            if provider.context_window:
                worker.env["PI_MODEL_CONTEXT_WINDOW"] = str(provider.context_window)
            LOG.info("injected model provider %s into worker %s", provider.name, worker.name)

    @staticmethod
    def _select_provider(worker: WorkerConfig, enabled: list[ModelProvider]) -> ModelProvider | None:
        pinned = (worker.env.get("PI_PROVIDER") or "").strip()
        if pinned:
            for provider in enabled:
                if provider.name == pinned:
                    return provider
            LOG.warning("worker %s pins unknown provider %s", worker.name, pinned)
            return None
        return enabled[0]
    ...
    def _run_local_binary_check(self) -> None:
        ...
        LOG.warning(
            ...
            "configured and usable directly (e.g. `claude -p ...` works) — 闪电（lightning） injects no API keys.",
