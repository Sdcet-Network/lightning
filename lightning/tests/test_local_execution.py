...
from lightning.dispatcher.config import DispatchConfig, LocalConfig, WorkerConfig
from lightning.dispatcher.runtime.local_backend import LocalBackend
from lightning.dispatcher.runtime.local_process import LocalProcess
from lightning.dispatcher.scheduler import loop as loop_module
from lightning.dispatcher.tasks import common, explore
from lightning.dispatcher.runtime.cancellation import TaskCancellation
from lightning.dispatcher.workers.adapters import pi as pi_module
from lightning.dispatcher.workers.adapters.codex import CodexDriver
from lightning.dispatcher.workers.adapters.pi import PiDriver
from lightning.dispatcher.workers.registry import get_driver
...
def test_local_backend_merges_host_env_with_worker_env(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv("LIGHTNING_HOST_VAR", "host")
    backend = LocalBackend(LocalConfig(workspace_root=str(tmp_path)))
    handle = backend.ensure_running("proj_001")

    process = backend.build_exec_process(
        handle,
        {"LIGHTNING_WORKER_VAR": "worker"},
        ["sh", "-c", 'printf "%s-%s" "$LIGHTNING_HOST_VAR" "$LIGHTNING_WORKER_VAR"'],
        timeout_seconds=10,
    )
    process.start()
    result = process.communicate(timeout=20)
...
def test_pi_local_driver_omits_models_json_and_provider() -> None:
    ...
    assert "exec " in argv[2] and "pi" in argv[2]
...
def test_inject_providers_into_pi_worker() -> None:
    config = DispatchConfig.model_validate(_local_payload())
    ...
    loop._inject_providers()

    pi_worker = next(worker for worker in config.workers if worker.type == "pi")
    assert pi_worker.env["PI_MODEL"] == "deepseek-v4"
    assert pi_worker.env["PI_BASE_URL"] == "https://api.example.com"
    assert pi_worker.env["PI_API_KEY"] == "sk-key"
    assert pi_worker.env["PI_PROVIDER_API"] == "openai-completions"


def test_inject_providers_pins_selected_provider() -> None:
    config = DispatchConfig.model_validate(_local_payload())
    config.workers[2].env["PI_PROVIDER"] = "b"
    loop = _bare_loop(config)
    loop.client = FakeClient(
        make_project(),
        providers=[
            ModelProvider(name="a", api="responses", base_url="http://a", model="a-model", api_key="k", enabled=True),
            ModelProvider(name="b", api="anthropic", base_url="http://b", model="b-model", api_key="k", enabled=True),
        ],
    )

    loop._inject_providers()

    pi_worker = config.workers[2]
    assert pi_worker.env["PI_MODEL"] == "b-model"
    assert pi_worker.env["PI_PROVIDER_API"] == "anthropic"


def test_inject_providers_skips_workers_with_explicit_env() -> None:
    config = DispatchConfig.model_validate(_local_payload())
    config.workers[2].env["PI_MODEL"] = "host-model"
    loop = _bare_loop(config)
    loop.client = FakeClient(
        make_project(),
        providers=[ModelProvider(name="a", api="responses", base_url="http://a", model="a-model", api_key="k")],
    )

    loop._inject_providers()

    assert config.workers[2].env["PI_MODEL"] == "host-model"
    assert "PI_API_KEY" not in config.workers[2].env


def test_inject_providers_noop_without_enabled_providers() -> None:
    config = DispatchConfig.model_validate(_local_payload())
    loop = _bare_loop(config)
    loop.client = FakeClient(
        make_project(),
        providers=[ModelProvider(name="a", api="responses", base_url="http://a", model="a-model", api_key="k", enabled=False)],
    )
...
def test_pi_local_driver_uses_injected_provider() -> None:
    ...
    assert argv[0] == "/bin/sh"
    assert "--provider" in argv and "lightning" in argv
    assert "--model" in argv and "deepseek-v4" in argv
    assert "models.json" in argv[2]


def test_pi_local_binary_resolves_bundled_when_not_on_path(monkeypatch) -> None:
    monkeypatch.setattr(pi_module.shutil, "which", lambda _name: None)

    binary = pi_module._resolve_pi_binary()

    assert binary.endswith("bundled/pi/node_modules/.bin/pi")
