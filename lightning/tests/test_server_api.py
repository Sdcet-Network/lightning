...
def test_providers_start_empty_and_round_trip(client: TestClient) -> None:
    assert client.get("/settings/providers").json() == []
    ...
    response = client.put("/settings/providers", json=providers)
    assert response.status_code == 200
    assert response.json() == providers

    assert client.get("/settings/providers").json() == providers

    assert client.put("/settings/providers", json=[]).json() == []
    assert client.get("/settings/providers").json() == []


def test_provider_rejects_empty_required_fields(client: TestClient) -> None:
    response = client.put(
        "/settings/providers",
        json=[{"name": "  ", "api": "responses", "base_url": "http://x", "model": "m", "api_key": "k"}],
    )
    assert response.status_code == 422
